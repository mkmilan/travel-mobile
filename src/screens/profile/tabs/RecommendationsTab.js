import TripCard from "@/src/components/TripCard"; // placeholder card
import { getRecommendationsByUser } from "@/src/services/api";
...
export default function RecommendationsTab({ userId }) {
	...
	useEffect(() => {
		...
		(async () => {
			try {
				const res = await getRecommendationsByUser(userId, page);
